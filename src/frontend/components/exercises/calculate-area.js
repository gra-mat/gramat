...
class CalculateArea extends LitElement {
    static properties = {
        ...
        square_side_length: {type: Number},
  ...
  static styles = css`
    ...
    .question { display: contents; font-size: 3rem; font-weight: bold; margin-bottom: 20px; }
  ...
  updated(changedProps) {
    ...
    return html`
      <div
        class="grid"
        style="grid-template-columns: repeat(${size}, 20px);"
      >
        ${cells}
      </div>
    `;
    ...
    renderExerciseQuestion() {
      switch (this.config.question_type) {
        case 'calculate_area_square':
          return html`
            <div>${this.exercise}</div>
            <div class="square-wrapper">
                <div class="top-label">${this.square_side_length}</div>
                <div class="left-label">${this.square_side_length}</div>
...
customElements.define("x-calculate-area", CalculateArea);